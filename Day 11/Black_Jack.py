...
def calculate_score(_cards):
    sum_of_cards =  sum(_cards)

    #Hint 7: Inside calculate_score() check for a blackjack (a hand with only 2 cards: ace + 10) and return 0 instead of the actual score. 0 will represent a blackjack in our game.
    if sum_of_cards == 21 and len(_cards) == 2:
        return 0

    #Hint 8: Inside calculate_score() check for an 11 (ace). If the score is already over 21, remove the 11 and replace it with a 1. You might need to look up append() and remove().
    if 11 in _cards and sum_of_cards > 21:
        _cards.remove(11)
        _cards.append(1)
    return sum_of_cards
...
def compare(_user_score, _computer_score):
    ...
    elif _computer_score > _user_score:
...
def play_game():
    ...
    play_more = input("Do you want to play Black Jack again? Type 'y' or 'n': ")
    while play_more == 'y':
        os.system("cls")
        play_game()
    else:
